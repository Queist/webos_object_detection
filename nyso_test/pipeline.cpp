...
GstElement *init_preprocess_bin() {
    ...
    // COPY THIS
    // use this to sink pad you want to set query function
    gst_pad_set_query_function(ghost_sink_pad, custom_query_allocation);
...
int objectDetectionPipeline(std::string url, bool use_object_detection, int gl_effect) {
    ...
    src_bin = init_src_bin(isfile, url.c_str());
...
int main(int argc, char **argv) {
    objectDetectionPipeline("/home/nyso/qwer.mp4", true, 10);
    return 0;
}
